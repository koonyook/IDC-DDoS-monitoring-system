...
class monflowd(Daemon):
	...
	def run(self):
		...
		if maxTime==None:
			...
			startTime=datetime.datetime.fromtimestamp(maxTime)+ datetime.timedelta(minutes=1)
		...
		while True:
			if cutSeconds(datetime.datetime.now())-startTime > datetime.timedelta(minutes=2):
				...
				for line in output:
					...
					flow=tmp[5]
					packet=tmp[7]
					byte=str(int(tmp[9])/1024)
					if ip in allIP:
						filteredIP.append((ip,flow))
						valueString+="(%s,'%s',%s,%s,%s),"%(startTime.strftime('%s'),ip,flow,packet,byte)
						#print ip,flow
				...
				if len(filteredIP)>0:
					...
					cur.execute("INSERT INTO flowLog (timestamp,ip,flow,packet,size) VALUES %s ;"%(valueString[0:-1]))
					
					thresholdTime=startTime-datetime.timedelta(days=setting.DELETE_THRESHOLD_DAY)
					cur.execute("DELETE FROM flowLog WHERE timestamp<%s"%(thresholdTime.strftime('%s')))
