...
con = MySQLdb.connect("203.146.64.37", "spy", "thisisaspy", "NetTracker")
cur = con.cursor()
	...
	if row is not None:
		...
		if brand=='Cisco':
			...
			while result.poll()==None:
				output+=result.communicate()[0]
			result.wait()
			#print "rcode:", result.returncode
			if result.returncode==0 and 'No Such' not in output:
				#can go on this mib
				output=output.split('\n')[0:-1]
				i=0
				while i<len(output):
	...
	for ifIndex in answer[E_ID]:
		if len(answer[E_ID][ifIndex])>0:
			...
			if cur.rowcount==1:
				...
				#print "INSERT INTO vlan_port (O_ID, vlanNumber) VALUES %s"%(valueString)
